...
public class GA {
    ...
    // ---------------- CONFIGURATION ----------------
    static final int POP_SIZE = 8;          // number of chromosomes
    static final int CHROM_LENGTH = 6;      // length of each chromosome
    static final int GENERATIONS = 10;      // total generations
    static final int MATING_POOL_SIZE = 4;  // number of parents selected
    static final double MUTATION_RATE = 0.1;

    static final String selection_type = "canonical"; // canonical, roulette, rank, tournament, steady
    static final String crossover_type = "single_point"; // single_point, two_point, uniform
    static final String mutation_type = "bit_flip";      // bit_flip, swap
    ...
    static List<String> canonical_selection(List<String> pop, List<Integer> fit, int k) {
        ...
        for (int f : fit) probs.add(f / avg_fit);

        double sum = probs.stream().mapToDouble(Double::doubleValue).sum();
        probs = probs.stream().map(p -> p / sum).collect(Collectors.toList());

        System.out.println("\n--- Canonical Selection Probabilities ---");
        for (int i = 0; i < pop.size(); i++) {
            System.out.printf("Chromosome: %s | Fitness: %d | Probability: %.3f%n", pop.get(i), fit.get(i), probs.get(i));
        }
    ...
    static List<String> rank_based_selection(List<String> pop, List<Integer> fit, int k) {
        ...
        for (int i = 0; i < fit.size(); i++) sortedIdx.add(i);
        sortedIdx.sort(Comparator.comparingInt(fit::get));

        int n = fit.size();
        List<Integer> ranks = new ArrayList<>();
        for (int i = 1; i <= n; i++) ranks.add(i);
        ...
        for (int i = 0; i < ranked_pop.size(); i++) {
            System.out.printf("Chromosome: %s | Rank: %d | Probability: %.3f%n",
                    ranked_pop.get(i), ranks.get(i), rank_probs.get(i));
        }
    ...
    static List<String> tournament_selection(List<String> pop, List<Integer> fit, int k, int t_size) {
        ...
        for (int i = 0; i < k; i++) {
            List<Integer> indices = random.ints(0, pop.size()).distinct().limit(t_size).boxed().collect(Collectors.toList());
            System.out.println("\nTournament " + (i + 1) + " random indices: " + indices);
            int bestIndex = indices.get(0);
            for (int idx : indices)
                if (fit.get(idx) > fit.get(bestIndex)) bestIndex = idx;
            selected.add(pop.get(bestIndex));
            System.out.println("Selected: " + pop.get(bestIndex) + " (Fitness: " + fit.get(bestIndex) + ")");
        }
        return selected;
    }
    ...
    static String[] single_point_crossover(String p1, String p2) {
        ...
        System.out.println("Single Point Crossover at index: " + pt);
        return new String[]{
    ...
    static String[] two_point_crossover(String p1, String p2) {
        ...
        System.out.println("Two Point Crossover between index: " + pt1 + " and " + pt2);
        return new String[]{
    ...
    // ---------------- WEIGHTED CHOICE (Now shows random numbers) ----------------
    static List<String> weighted_choice(List<String> items, List<Double> weights, int k) {
        ...
        for (int i = 0; i < k; i++) {
            ...
            for (int j = 0; j < items.size(); j++) {
                cum += weights.get(j);
                if (r <= cum) {
                    System.out.printf("Random = %.3f → Selected: %s%n", r, items.get(j));
    ...
    public static void main(String[] args) {
        System.out.println("===== GENETIC ALGORITHM SIMULATION =====\n");
        ...
        for (int gen = 0; gen < GENERATIONS; gen++) {
            ...
            int bestFitness = Collections.max(fitness);
            int bestIndex = fitness.indexOf(bestFitness);
            String elite = population.get(bestIndex);
            double avg = fitness.stream().mapToInt(f -> f).average().orElse(0.0);

            for (int i = 0; i < population.size(); i++) {
                System.out.printf("Chromosome: %s | Fitness: %d%n", population.get(i), fitness.get(i));
            }
            System.out.printf(">> Best Fitness = %d | Average Fitness = %.2f%n", bestFitness, avg);
            System.out.println(">> Elitism selected: " + elite);
            ...
            next_gen.add(elite); // include elitism
            for (int i = 0; i < parents.size() - 1; i += 2) {
                String[] children = crossover_method(parents.get(i), parents.get(i + 1), crossover_type);
                String child1 = mutation_method(children[0], mutation_type);
                String child2 = mutation_method(children[1], mutation_type);
                next_gen.add(child1);
                next_gen.add(child2);
                System.out.printf("\nParents: %s + %s -> Children: %s , %s%n",
                        parents.get(i), parents.get(i + 1), child1, child2);
            }
        ...
        // Final Output
        List<Integer> finalFitness = population.stream().map(GA::fitness_function).collect(Collectors.toList());
        int bestIndex = finalFitness.indexOf(Collections.max(finalFitness));
        System.out.println("===== FINAL BEST CHROMOSOME =====");
        System.out.println("Best Chromosome: " + population.get(bestIndex) + " | Fitness: " + finalFitness.get(bestIndex));
    }

    // ---------------- SELECTION WRAPPER ----------------
    static List<String> select_method(List<String> pop, List<Integer> fit, String method) {
        switch (method) {
            case "canonical": return canonical_selection(pop, fit, MATING_POOL_SIZE);
            case "roulette": return roulette_wheel_selection(pop, fit, MATING_POOL_SIZE);
            case "rank": return rank_based_selection(pop, fit, MATING_POOL_SIZE);
            case "tournament": return tournament_selection(pop, fit, MATING_POOL_SIZE, 3);
            case "steady": return steady_state_selection(pop, fit, MATING_POOL_SIZE);
    ...
    static String[] crossover_method(String p1, String p2, String method) {
        switch (method) {
            case "single_point": return single_point_crossover(p1, p2);
            case "two_point": return two_point_crossover(p1, p2);
            case "uniform": return uniform_crossover(p1, p2);
            default: return new String[]{p1, p2};
        }
    }

    static String mutation_method(String chrom, String method) {
        switch (method) {
            case "bit_flip": return bit_flip_mutation(chrom, MUTATION_RATE);
            case "swap": return swap_mutation(chrom);
